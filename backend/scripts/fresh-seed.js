...
const freshSeedData = async () => {
  try {
    console.log('🌱 Starting FRESH seed data creation...');
    console.log('⚠️  This will CLEAR all existing data and create new data.\n');
    ...
    // Create Goa-centric itineraries
    const itineraries = [
      {
    ...
    await Itinerary.findByIdAndUpdate(createdItineraries[2]._id, {
      blocks: [createdBlocks[5]._id, createdBlocks[6]._id, createdBlocks[7]._id]
    });

    await Itinerary.findByIdAndUpdate(createdItineraries[3]._id, {
      blocks: [createdBlocks[8]._id, createdBlocks[9]._id]
    });

    await Itinerary.findByIdAndUpdate(createdItineraries[4]._id, {
      blocks: [createdBlocks[10]._id, createdBlocks[11]._id, createdBlocks[12]._id]
    });

    await Itinerary.findByIdAndUpdate(createdItineraries[5]._id, {
      blocks: [createdBlocks[13]._id, createdBlocks[14]._id]
    });

    await Itinerary.findByIdAndUpdate(createdItineraries[6]._id, {
      blocks: [createdBlocks[15]._id, createdBlocks[16]._id]
    });

    await Itinerary.findByIdAndUpdate(createdItineraries[7]._id, {
      blocks: [createdBlocks[17]._id, createdBlocks[18]._id]
    });

    await Itinerary.findByIdAndUpdate(createdItineraries[8]._id, {
      blocks: [createdBlocks[19]._id]
    });

    await Itinerary.findByIdAndUpdate(createdItineraries[9]._id, {
      blocks: [createdBlocks[20]._id, createdBlocks[21]._id]
    });
